...
# --- PAGE CONFIGURATION ---
st.set_page_config(page_title="Leave Management System", page_icon="📅", layout="wide")
...
def load_users_data():
    try:
        df = conn.read(worksheet="Users", usecols=["Name", "PIN", "Role", "Post", "JoiningDate"])
...
def inject_balance_formulas(df):
    ...
    for i in range(len(df_copy)):
        r = i + 2
        df_copy.at[df_copy.index[i], "Post"] = f'=IFERROR(VLOOKUP(A{r}, Users!A:E, 4, FALSE), "")'
        df_copy.at[df_copy.index[i], "JoiningDate"] = f'=IFERROR(IF(VLOOKUP(A{r}, Users!A:E, 5, FALSE)="", "", TEXT(VLOOKUP(A{r}, Users!A:E, 5, FALSE), "yyyy-mm-dd")), "")'
        
        df_copy.at[df_copy.index[i], "Service Year"] = f'=IF(C{r}="", 0, TO_PURE_NUMBER((TODAY() - DATEVALUE(C{r}))/365.25))'
        df_copy.at[df_copy.index[i], "Service Year Penalty"] = f"=AD{r} + AC{r} + Y{r} + AA{r}"
        df_copy.at[df_copy.index[i], "Actual Service Year"] = f'=IF(C{r}="", 0, TO_PURE_NUMBER((TODAY() - (DATEVALUE(C{r}) + E{r}))/365.25))'
        
        df_copy.at[df_copy.index[i], "Addition_UL"] = f"=IF(AND(F{r}>=5, MOD(FLOOR(F{r}), 5)=0), 30, 0)"
        df_copy.at[df_copy.index[i], "Last_UL_Milestone"] = f"=FLOOR(F{r}/5)*5"
        
        df_copy.at[df_copy.index[i], "Penalty_CL(UL Effect)"] = f"=ROUND(((Y{r}+AC{r})/30)*0.75*2)/2"
        df_copy.at[df_copy.index[i], "Penalty_SL(UL Effect)"] = f"=ROUND(((Y{r}+AC{r})/30)*1.25*2)/2"
        df_copy.at[df_copy.index[i], "Penalty_AL(UL Effect)"] = f"=ROUND(((Y{r}+AC{r})/30)*2.5*2)/2"
        
        df_copy.at[df_copy.index[i], "Balance_CL"] = f"=G{r}+H{r}-I{r}-J{r}"
        df_copy.at[df_copy.index[i], "Balance_SL"] = f"=L{r}+M{r}-N{r}-O{r}"
        df_copy.at[df_copy.index[i], "Balance_AL"] = f"=MIN(180, Q{r}+R{r}-S{r}-T{r}-U{r})"
...
def generate_leave_pdf(row):
    try:
        ...
        pdf.cell(0, 10, "སྤྱི་ཁྱབ་རྩིས་ཞིབ་ལས་ཁང་།", new_x="LMARGIN", new_y="NEXT", align="C")
        pdf.cell(0, 10, "(དགོངས་ཞུ་འགེངས་ཤོག)", new_x="LMARGIN", new_y="NEXT", align="C")
        ...
        if staff_df is not None and not staff_df.empty:
            ...
            if not user_data.empty:
                ...
                post = str(user_data.iloc[0].get("Post", "") if "Post" in user_data.columns else "")
                if post == "nan": post = ""
                if joining_date == "nan" or joining_date == "NaT": joining_date = ""
        
        pdf.cell(95, 10, f"༡། མཚན། {row.get('Name', '')}")
        pdf.cell(95, 10, f"༢། ལས་བྱེད་དུ་བསྐོ་གཞག་ཞུས་ཚེས། {to_tibetan_numeral(format_tibetan_date(joining_date))}", new_x="LMARGIN", new_y="NEXT")
        
        pdf.cell(95, 10, f"༣། གནས་རིམ།/གོ་གནས། {to_tibetan_numeral(post)}")
        pdf.cell(95, 10, f"༤། ལས་ཁུངས་དང་སྡེ་ཚན། {row.get('Department', '')}", new_x="LMARGIN", new_y="NEXT")
        
        pdf.ln(2)
        start = str(row.get('StartDate', ''))
        end = str(row.get('EndDate', ''))
        def get_tibetan_half(half_val):
            if not half_val or pd.isna(half_val): return ""
            if "Morning" in str(half_val): return " (ཞོགས་པ།)"
            if "Evening" in str(half_val): return " (ཉིན་རྒྱབ།)"
            if "Full Day" in str(half_val): return " (ཉིན་ཆ་ཚང་།)"
            return ""
        ...
        pdf.cell(0, 10, to_tibetan_numeral(date_str), new_x="LMARGIN", new_y="NEXT")
        
        pdf.cell(0, 10, f"གུང་གསེང་ཉིན་གྲངས་ཇི་ཞུས། {to_tibetan_numeral(row.get('TotalDays', ''))}", new_x="LMARGIN", new_y="NEXT")
        ...
        pdf.set_font("Monlam", "", 12)
        pdf.cell(0, 10, "༦། གུང་གསེང་ལྷག་བསྡད་གནས་སྟངས་གཤམ་གསལ།", new_x="LMARGIN", new_y="NEXT")
        
        bal = calculate_balances(row.get('Name', ''))
        
        pdf.cell(95, 10, f"ཀ། ངེས་མེད། {to_tibetan_numeral(bal['CL'])}")
        pdf.cell(95, 10, f"ཁ། ཐོབ་སེང་། {to_tibetan_numeral(bal['AL'])}", new_x="LMARGIN", new_y="NEXT")
        
        pdf.cell(95, 10, f"ག། ནད་དགོངས། {to_tibetan_numeral(bal['SL'])}")
        pdf.cell(95, 10, f"ང། དམིགས་བསལ་ཕོགས་མེད། {to_tibetan_numeral(bal['UL'])}", new_x="LMARGIN", new_y="NEXT")
        
        pdf.cell(0, 10, f"ཅ། གཞན། {to_tibetan_numeral(row.get('Reason', ''))}", new_x="LMARGIN", new_y="NEXT")
        
        app_date = str(row.get('ApplicationDate', ''))
        if app_date and app_date != "nan":
            pdf.cell(0, 10, f"༧། སྙན་ཞུ་ཕུལ་བའི་ཚེས། {to_tibetan_numeral(format_tibetan_date(app_date))}", new_x="LMARGIN", new_y="NEXT")
        ...
        pdf.set_xy(20, y + 2)
        pdf.multi_cell(75, 6, "དགོངས་སྙན་འབུལ་མཁན་དོ་བདག་ལས་བྱེད་ཀྱི་ས་རྟགས།", align="C")
        ...
        pdf.set_xy(20, y + 2)
        pdf.multi_cell(75, 6, "བཀའ་འཁྲོལ་བརྒྱབ་གཉེར།\n(འབྲེལ་ཡོད་ཟུང་དྲུང་།)", align="C")
        ...
        pdf.set_xy(115, y + 2)
        pdf.multi_cell(75, 6, "བཀའ་འཁྲོལ་ཆོག་མཆན།\nའགན་འཛིན།/དྲུང་ཆེ།", align="C")
...
def login_screen():
    ...
    if users_df is not None and not users_df.empty:
        ...
        with st.form("login_form"):
            ...
            if submit_button:
                user_record = users_df[users_df["Name"] == selected_name].iloc[0]
        ...
        st.warning("No users found in the database or database connection failed.")
...
def employee_dashboard(hide_title=False):
    ...
    try:
        ...
        if actual_service_year < 0:
            ...
            st.metric("Actual Service Tenure", f"{years} Years, {months} Months, {days} Days")
            
            penalty_days = balances.get("Service_Year_Penalty", 0)
            joining_date = balances.get("JoiningDate", "N/A")
            st.caption(f"Joining Date: {joining_date} | Service Penalties Applied: {penalty_days} days")
    except Exception as e:
    ...
    action = st.radio("Select Action:", ["Request Leave", "Leave Reversal / Return Early"], horizontal=True)
    ...
    leave_type_selection = st.selectbox("Leave Type", list(leave_options.values()))
    leave_type = [k for k, v in leave_options.items() if v == leave_type_selection][0]
    ...
    selected_coadmin = st.selectbox("Select Co-Admin for Support", ["None"] + co_admins)
    
    application_date = st.date_input("Application Date", datetime.today())
    
    col_start, col_end = st.columns(2)
    start_date = col_start.date_input("Start Date")
    start_half = col_start.radio("Start Time", ["Full Day ཉིན་ཆ་ཚང་།", "Morning ཞོགས་པ།", "Evening ཉིན་རྒྱབ།"], index=0, horizontal=True)
    end_date = col_end.date_input("End Date")
    end_half = col_end.radio("End Time", ["Full Day ཉིན་ཆ་ཚང་།", "Morning ཞོགས་པ།", "Evening ཉིན་རྒྱབ།"], index=0, horizontal=True)
    ...
    total_days = st.number_input("Total Days (to request or restore)", min_value=min_allowed, max_value=max_allowed, value=max_allowed, step=0.5)
    ...
    if col_sub.button("Submit Reversal" if is_reversal else "Submit Request", type="primary"):
        if total_days <= 0:
            ...
            st.error("Total days must be at least 1 when your leave spans multiple days.")
        else:
            try:
                ...
                updated_df = pd.concat([df_requests, new_request], ignore_index=True)
    ...
    if not df_requests.empty and "Name" in df_requests.columns:
        user_requests = df_requests[df_requests["Name"] == st.session_state.user_name]
        if not user_requests.empty:
            ...
            if not pending.empty:
                ...
                for idx, row in pending.iterrows():
                    ...
                    col1.write(f"**{row['LeaveType']}** for {row['TotalDays']} days ({row['StartDate']} to {row['EndDate']})")
            ...
            if not others.empty:
                ...
                for idx, row in others.iterrows():
                    col1, col2 = st.columns([5, 1])
                    col1.write(f"**{row['LeaveType']}** for {row['TotalDays']} days ({row['StartDate']} to {row['EndDate']}) - Status: {row['Status']}")
...
def leave_accounting_engine():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["FY Rollover", "AL Encashment", "Add User", "Leave Statement", "Factory Reset"])
    ...
    with tab1:
        ...
        archive_name = st.text_input("Name of ending Financial Year (e.g. FY2025-2026) for Archive", "FY2025-2026")
            ...
            if staff_df is not None and not staff_df.empty:
                try:
                    for idx, row in staff_df.iterrows():
                        ...
                        cum_ul = s_float(row.get("Cumulative_UL", 0)) if "Cumulative_UL" in staff_df.columns else 0
                        staff_df.at[idx, "Cumulative_UL"] = cum_ul + used_ul_this_year
                        curr_eff = s_float(row.get("Current Year Leave affecting Service Year", 0)) if "Current Year Leave affecting Service Year" in staff_df.columns else 0
                        cum_eff = s_float(row.get("Cummulative leave effecting Service year.(Education Leave & Etc).", 0)) if "Cummulative leave effecting Service year.(Education Leave & Etc)." in staff_df.columns else 0
                        staff_df.at[idx, "Cummulative leave effecting Service year.(Education Leave & Etc)."] = cum_eff + curr_eff
                        
                        if "Current Year Leave affecting Service Year" in staff_df.columns:
                            staff_df.at[idx, "Current Year Leave affecting Service Year"] = 0.0
                        
                        # Recalculate exactly to prevent double counting
                        effect_days = s_float(staff_df.at[idx, "Cummulative leave effecting Service year.(Education Leave & Etc)."])
                        ...
                        staff_df.at[idx, "Service Year Penalty"] = service_penalty
                        
                        ul_addition = 0.0
                        
                        if "Actual Service Year" not in staff_df.columns: staff_df["Actual Service Year"] = 0.0
                        if "Service Year" not in staff_df.columns: staff_df["Service Year"] = 0.0
                        staff_df["Actual Service Year"] = staff_df["Actual Service Year"].astype(float)
                        staff_df["Service Year"] = staff_df["Service Year"].astype(float)
                        ...
                        try:
                            ...
                            calendar_service_years = (datetime.now() - joining_date).days / 365.25
                            staff_df.loc[idx, "Service Year"] = round(calendar_service_years, 2)
                            
                            if service_penalty == 0:
                                tenure_years = calendar_service_years
                            else:
                                true_service_start = joining_date + timedelta(days=service_penalty)
                                tenure_years = (datetime.now() - true_service_start).days / 365.25
                            staff_df.loc[idx, "Actual Service Year"] = round(tenure_years, 2)
                            
                            last_ms = s_float(row.get("Last_UL_Milestone", 0)) if "Last_UL_Milestone" in staff_df.columns else 0
                    ...
                    fixed_cols = ["Name", "Post", "JoiningDate", "Service Year", "Service Year Penalty", "Actual Service Year"]
                    cl_cols = ["Opening_CL", "Addition_CL", "Penalty_CL(UL Effect)", "Used_CL", "Balance_CL"]
                    sl_cols = ["Opening_SL", "Addition_SL", "Penalty_SL(UL Effect)", "Used_SL", "Balance_SL"]
                    al_cols = ["Opening_AL", "Addition_AL", "Penalty_AL(UL Effect)", "Used_AL", "Encashed_AL", "Balance_AL"]
                    ul_cols = ["Opening_UL", "Addition_UL", "Used_UL", "Balance_UL", "Cumulative_UL", "Last_UL_Milestone", "Current Year Leave affecting Service Year", "Cummulative leave effecting Service year.(Education Leave & Etc)."]
                    
                    all_ordered_cols = [c for c in fixed_cols + cl_cols + sl_cols + al_cols + ul_cols if c in staff_df.columns]
                    other_cols = [c for c in staff_df.columns if c not in all_ordered_cols]
                    ...
                    try:
                        ...
                        if df_req is not None and not df_req.empty:
                            ...
                            try:
                                spread = conn.client._open_spreadsheet(spreadsheet=None, folder_id=None)
                                try:
                                    spread.worksheet(archive_sheet)
                                except Exception:
                                    spread.add_worksheet(title=archive_sheet, rows="1000", cols="20")
                            except Exception as e:
                                st.warning(f"Could not check/create worksheet structure, attempting direct update. Error: {e}")
                                
                            conn.update(worksheet=archive_sheet, data=df_req)
                            
                            # Wipe main requests sheet
                            empty_req = pd.DataFrame(columns=df_req.columns)
                            conn.update(worksheet="LeaveRequests", data=empty_req)
                    except Exception as archive_e:
                        st.error(f"Archiving failed: {archive_e}")
                        
                    st.cache_data.clear()
                    st.success(f"FY Rollover Successful! All data archived to {archive_sheet}.")
                    st.rerun()
                except Exception as e:
    ...
    with tab2:
        ...
        if staff_df is not None and not staff_df.empty:
            encash_user = st.selectbox("Select Employee", staff_df["Name"].tolist(), key="encash_user")
            encash_days = st.number_input("Days to Encash", min_value=0.5, step=0.5)
            if st.button("Process Encashment"):
                idx = staff_df[staff_df["Name"] == encash_user].index[0]
                curr_al = float(staff_df.at[idx, "Balance_AL"]) if pd.notnull(staff_df.at[idx, "Balance_AL"]) else 0
                
                if encash_days > curr_al:
                    st.error("Cannot encash more days than the current AL balance.")
                else:
                    curr_encashed = float(staff_df.at[idx, "Encashed_AL"]) if "Encashed_AL" in staff_df.columns and pd.notnull(staff_df.at[idx, "Encashed_AL"]) else 0
                    staff_df.at[idx, "Encashed_AL"] = curr_encashed + encash_days
                    
                    conn.update(worksheet="Staff_Master", data=inject_balance_formulas(staff_df))
                    st.cache_data.clear()
                    st.success(f"Successfully encashed {encash_days} AL days for {encash_user}.")
                    st.rerun()
                    
    with tab3:
        st.subheader("Add New User")
        with st.form("add_user_form"):
            new_name = st.text_input("Full Name (Type in Tibetan Font to show it in Leave Form that is in Tibetan)")
            new_post = st.text_input("Post (Type in Tibetan Font to show it in Leave Form that is in Tibetan)")
            new_join = st.date_input("Joining Date")
            new_pin = st.text_input("4-Digit PIN", type="password", max_chars=4)
            new_role = st.selectbox("Role", ["Employee", "Admin", "Co-Admin", "Accounts"])
            ...
            if st.form_submit_button("Add User"):
                ...
                if len(new_pin) != 4 or not new_pin.isdigit():
                    ...
                    if new_name in users_df["Name"].values:
                        ...
                        # Prorate calculation
                        fy_end = datetime(new_join.year if new_join.month < 4 else new_join.year + 1, 3, 31).date()
                        ...
                        new_user_row = pd.DataFrame([{"Name": new_name, "PIN": new_pin, "Role": new_role, "Post": new_post, "JoiningDate": new_join.strftime("%Y-%m-%d")}])
                        
                        conn.update(worksheet="Staff_Master", data=inject_balance_formulas(pd.concat([staff_df, new_staff_row], ignore_index=True)))
                        conn.update(worksheet="Users", data=pd.concat([users_df, new_user_row], ignore_index=True))
                        st.cache_data.clear()
                        st.success(f"User {new_name} added successfully with prorated balances!")
    ...
    with tab4:
        ...
        if staff_df is not None and not staff_df.empty:
            ...
            with col1:
                ...
                export_user = st.selectbox("Select Employee", staff_df["Name"].tolist(), key="exp_user")
                ...
                with pd.ExcelWriter(buffer_single, engine='openpyxl') as writer:
                    stmt_df.to_excel(writer, index=False, sheet_name='Leave Statement')
            ...
            with col2:
                ...
                st.write("Generates a single master Excel table containing all employees and their complete leave balances.")
                ...
                with pd.ExcelWriter(buffer_all, engine='openpyxl') as writer:
                    ...
                    export_cols = [c for c in fixed_cols + cl_cols + sl_cols + al_cols + ul_cols if c in staff_df.columns]
                    export_df = staff_df[export_cols]
                    export_df.to_excel(writer, index=False, sheet_name="Master Leave Statement")
        ...
        if staff_df is not None and not staff_df.empty:
            if st.button("🚨 WIPE ALL LEAVE DATA TO 0", type="primary"):
                try:
                    ...
                    conn.update(worksheet="Staff_Master", data=inject_balance_formulas(staff_df))
                    st.cache_data.clear()
                    st.success("Successfully wiped all data! Everything is exactly 0.")
                    st.rerun()
                except Exception as e:
    ...
    if st.button("🔄 Calculate Actual Service Year", type="primary"):
        ...
        if staff_df is not None and not staff_df.empty:
            try:
                ...
                staff_df["Actual Service Year"] = staff_df["Actual Service Year"].astype(float)
                staff_df["Service Year"] = staff_df["Service Year"].astype(float)
                staff_df["Service Year Penalty"] = staff_df["Service Year Penalty"].astype(float)
                ...
                for idx, row in staff_df.iterrows():
                    try:
                        ...
                        effect_days_curr = s_float(row.get("Current Year Leave affecting Service Year", 0))
                        effect_days_cum = s_float(row.get("Cummulative leave effecting Service year.(Education Leave & Etc).", 0))
                        ...
                        staff_df.loc[idx, "Service Year Penalty"] = service_penalty
                        ...
                        if ul_addition > 0:
                            curr_add = s_float(row.get("Addition_UL", 0)) if "Addition_UL" in staff_df.columns else 0
                            staff_df.loc[idx, "Addition_UL"] = curr_add + ul_addition
                ...
                print("DEBUG STAFF_DF:", staff_df[["Actual Service Year"]].head())
    ...
    try:
        ...
        if fy_df is not None and not fy_df.empty:
            ...
            with st.form("fy_form"):
                new_start = st.text_input("Start Date (e.g. 2025-04-01)", value=current_start)
                new_end = st.text_input("End Date (e.g. 2026-03-31)", value=current_end)
...
def admin_dashboard():
    ...
    if staff_df is not None and not staff_df.empty:
        ...
        st.dataframe(pd.DataFrame(balances_list), width="stretch", hide_index=True)
    else:
                    ...
                    if col1.button("Approve", key=f"approve_{row['ID']}"):
                        ...
                        if staff_df is not None:
                            ...
                            if not employee_idx.empty:
                                ...
                                # Only update Staff_Master for leaves that are tracked
                                ...
                                # ALWAYS recalculate Service Year Penalty so UL updates are captured instantly
...
def coadmin_dashboard():
    ...
    if not df_requests.empty and "SelectedCoAdmin" in df_requests.columns:
        ...
        if to_review.empty:
            ...
            st.error(f"🔔 You have {len(to_review)} leave request(s) waiting for your support!")
            for idx, row in to_review.iterrows():
                with st.expander(f"{row['Name']} requested {row['TotalDays']} days of {row['LeaveType']}"):
                    ...
                    if st.button("Support Request", type="primary", key=f"ack_{row['ID']}"):
                        df_requests.at[idx, "CoAdminAcknowledged"] = "Supported"
                        df_requests.at[idx, "SupportedBy"] = st.session_state.user_name
                        conn.update(worksheet="LeaveRequests", data=df_requests)
                        st.cache_data.clear()
                        st.rerun()
    else:
...
def main_dashboard():
    ...
    if users_df is not None and staff_df is not None:
        missing_users = []
        staff_names = staff_df["Name"].dropna().tolist() if "Name" in staff_df.columns else []
        ...
        if st.button("Update PIN"):
            if len(new_pin) != 4 or not new_pin.isdigit():
                ...
                if users_df is not None:
                    user_idx = users_df[users_df["Name"] == st.session_state.user_name].index
